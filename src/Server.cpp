...
void Server::removeChannel(int fd){
	for (size_t i = 0; i < this->channels.size(); i++){
		...
		if (flag){
			std::string rpl = ":" + getClientFromFd(fd)->getNickname() + "!~" + getClientFromFd(fd)->getUsername() + "@localhost quitCommand Quit\r\n";
...
void Server::receiveNewData(int fd){
	...
	if (bytes <= 0)	{
		...
		if (getClientFromFd(fd))
			getClientFromFd(fd)->clearBuffer();
	}
}
...
bool Server::isRegistered(int fd){
	if (!getClientFromFd(fd) || getClientFromFd(fd)->getNickname().empty() || getClientFromFd(fd)->getUsername().empty() || getClientFromFd(fd)->getNickname() == "*" || !getClientFromFd(fd)->getIsLoggedIn())
		return false;
	return true;
}
...
void Server::parseAndExecCommand(std::string &cmd, int fd){
	...
	if (splittedCmd.size() && (splittedCmd[0] == "BONG"))
		return;
	...
	else if (isRegistered(fd))
	{
		if (splittedCmd.size() && (splittedCmd[0] == "KICK"))
			...
			sendResponse(ERR_CMDNOTFOUND(getClientFromFd(fd)->getNickname(), splittedCmd[0]), fd);
	}
	else if (!isRegistered(fd))
