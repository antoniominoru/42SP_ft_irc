...
void Server::inviteCommand(std::string &cmd, int &fd){
	...
	if (scmd.size() < 3){
		sendError(461, getClientFromFd(fd)->getNickname(), fd, " :Not enough parameters\r\n");
		return;
	}
	...
	if (!(getChannel(channelname)->getClientFromFd(fd)) && !(getChannel(channelname)->getAdmin(fd))){
		sendError(442, channelname, fd, " :You're not on that channel\r\n");
		return;
	}
	if (getChannel(channelname)->getClientInChannel(scmd[1])){
		sendError(443, getClientFromFd(fd)->getNickname(), channelname, fd, " :is already on channel\r\n");
		return;
	}
	...
	if (getChannel(channelname)->getInviteOnly() && !getChannel(channelname)->getAdmin(fd)){
		sendError(482, getChannel(channelname)->getClientFromFd(fd)->getNickname(), scmd[1], fd, " :You're not channel operator\r\n");
		return;
	}
	if (getChannel(channelname)->getLimit() && getChannel(channelname)->getClientCount() >= getChannel(channelname)->getLimit()){
		sendError(473, getChannel(channelname)->getClientFromFd(fd)->getNickname(), channelname, fd, " :Cannot invit to channel (+i)\r\n");
		return;
	}
	clt->addChannelInvite(channelname);
	std::string rep1 = ": 341 " + getClientFromFd(fd)->getNickname() + " " + clt->getNickname() + " " + scmd[2] + "\r\n";
	sendResponse(rep1, fd);
	std::string rep2 = ":" + clt->getHostname() + " INVITE " + clt->getNickname() + " " + scmd[2] + "\r\n";
	sendResponse(rep2, clt->getFd());
}

int Server::parseJoinCommand(std::vector<std::pair<std::string, std::string> > &token, std::string cmd, int fd){
	...
	for (size_t i = 0; i < token.size(); i++){
		if (*(token[i].first.begin()) != '#'){
			sendError(403, getClientFromFd(fd)->getNickname(), token[i].first, getClientFromFd(fd)->getFd(), " :No such channel\r\n");
			token.erase(token.begin() + i--);
		}
		else
...
void Server::joinExistingChannel(std::vector<std::pair<std::string, std::string> > &token, int i, int j, int fd){
	...
	if (countJoinedChannels(getClientFromFd(fd)->getNickname()) >= 10){
		sendError(405, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :You have joined too many channels\r\n");
		return;
	}
	if (!this->channels[j].getPassword().empty() && this->channels[j].getPassword() != token[i].second){
		if (!IsInvited(getClientFromFd(fd), token[i].first, 0)){
			sendError(475, getClientFromFd(fd)->getNickname(), "#" + token[i].first, getClientFromFd(fd)->getFd(), " :Cannot join channel (+k) - bad key\r\n");
			return;
		}
	}
	if (this->channels[j].getInviteOnly()){
		if (!IsInvited(getClientFromFd(fd), token[i].first, 1)){
			sendError(473, getClientFromFd(fd)->getNickname(), "#" + token[i].first, getClientFromFd(fd)->getFd(), " :Cannot join channel (+i)\r\n");
			return;
		}
	}
	if (this->channels[j].getLimit() && this->channels[j].getClientCount() >= this->channels[j].getLimit()){
		sendError(471, getClientFromFd(fd)->getNickname(), "#" + token[i].first, getClientFromFd(fd)->getFd(), " :Cannot join channel (+l)\r\n");
		return;
	}
	Client *cli = getClientFromFd(fd);
	this->channels[j].addClient(*cli);
	if (channels[j].getTopicName().empty())
		sendResponse(RPL_JOINMSG(getClientFromFd(fd)->getHostname(), getClientFromFd(fd)->getIpAdd(), token[i].first) +
						  RPL_NAMREPLY(getClientFromFd(fd)->getNickname(), channels[j].getName(), channels[j].clientChannelList()) +
						  RPL_ENDOFNAMES(getClientFromFd(fd)->getNickname(), channels[j].getName()),
					  	fd);
	else
		sendResponse(RPL_JOINMSG(getClientFromFd(fd)->getHostname(), getClientFromFd(fd)->getIpAdd(), token[i].first) +
						  RPL_TOPICIS(getClientFromFd(fd)->getNickname(), channels[j].getName(), channels[j].getTopicName()) +
						  RPL_NAMREPLY(getClientFromFd(fd)->getNickname(), channels[j].getName(), channels[j].clientChannelList()) +
						  RPL_ENDOFNAMES(getClientFromFd(fd)->getNickname(), channels[j].getName()),
					  	fd);
	channels[j].sendToAll(RPL_JOINMSG(getClientFromFd(fd)->getHostname(), getClientFromFd(fd)->getIpAdd(), token[i].first), fd);
}

void Server::joinNonExistingChannel(std::vector<std::pair<std::string, std::string> > &token, int i, int fd){
	if (countJoinedChannels(getClientFromFd(fd)->getNickname()) >= 10){
		sendError(405, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :You have joined too many channels\r\n");
		return;
	}
	...
	sendResponse(RPL_JOINMSG(getClientFromFd(fd)->getHostname(), getClientFromFd(fd)->getIpAdd(), newChannel.getName()) +
					  RPL_NAMREPLY(getClientFromFd(fd)->getNickname(), newChannel.getName(), newChannel.clientChannelList()) +
					  RPL_ENDOFNAMES(getClientFromFd(fd)->getNickname(), newChannel.getName()),
				  fd);
}
...
void Server::joinCommand(std::string cmd, int fd){
	...
	if (!parseJoinCommand(token, cmd, fd)) {
		sendError(461, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :Not enough parameters\r\n");
		return;
	}
	if (token.size() > 10) {
		sendError(407, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :Too many channels\r\n");
		return;
	}
	for (size_t i = 0; i < token.size(); i++) {
		bool flag = false;
		...
		if (!flag)
			joinNonExistingChannel(token, i, fd);
	}
}
...
std::string Server::parseKickCommand(std::string cmd, std::vector<std::string> &tmp, std::string &user, int fd){
	...
	for (size_t i = 0; i < tmp.size(); i++)	{
		if (*(tmp[i].begin()) == '#')
			...
			sendError(403, getClientFromFd(fd)->getNickname(), tmp[i], getClientFromFd(fd)->getFd(), " :No such channel\r\n");
			tmp.erase(tmp.begin() + i--);
		}
	}
	return reason;
	...
	for (size_t i = 0; i < tmp.size(); i++){
		if (getChannel(tmp[i])){
			...
			if (!ch->getClientFromFd(fd) && !ch->getAdmin(fd)) {
				sendError(442, getClientFromFd(fd)->getNickname(), "#" + tmp[i], getClientFromFd(fd)->getFd(), " :You're not on that channel\r\n");
				continue;
			}
			if (ch->getAdmin(fd)){
				if (ch->getClientInChannel(user)){
					std::stringstream ss;
					ss << ":" << getClientFromFd(fd)->getNickname() << "!~" << getClientFromFd(fd)->getUsername() << "@"
					   << "localhost" << " kickCommand #" << tmp[i] << " " << user;
					if (!reason.empty())
						ss << " :" << reason << "\r\n";
					else
						ss << "\r\n";
					...
					sendError(441, getClientFromFd(fd)->getNickname(), "#" + tmp[i], getClientFromFd(fd)->getFd(), " :They aren't on that channel\r\n");
					continue;
				}
			}
			else {
				sendError(482, getClientFromFd(fd)->getNickname(), "#" + tmp[i], getClientFromFd(fd)->getFd(), " :You're not channel operator\r\n");
				continue;
			}
		}
		else
...
void Server::getCommandArgs(std::string cmd, std::string &name, std::string &modeset, std::string &params){
...
void Server::modeCommand(std::string &cmd, int fd){
	...
	if (channelName[0] != '#' || !(channel = getChannel(channelName.substr(1)))){
		...
		sendError(442, getClientFromFd(fd)->getNickname(), channelName, getClientFromFd(fd)->getFd(), " :You're not on that channel\r\n");
		return;
	} else if (modeset.empty()){
		sendResponse(RPL_CHANNELMODES(cli->getNickname(), channel->getName(), channel->getModes()) +
						  RPL_CREATIONTIME(cli->getNickname(), channel->getName(), channel->getCreationTime()),
					  fd);
		return;
		...
		for (size_t i = 0; i < modeset.size(); i++)	{
			if (modeset[i] == '+' || modeset[i] == '-')
				...
				if (modeset[i] == 'i')
					...
					mode_chain << topicRestrictionOption(channel, opera, mode_chain.str());
				else if (modeset[i] == 'k')
					mode_chain << channelKeyOption(tokens, channel, pos, opera, fd, mode_chain.str(), arguments);
				else if (modeset[i] == 'o')
					mode_chain << operatorPrivilegeOption(tokens, channel, pos, fd, opera, mode_chain.str(), arguments);
				else if (modeset[i] == 'l')
					mode_chain << userLimitOption(tokens, channel, pos, opera, fd, mode_chain.str(), arguments);
				else
...
std::string Server::inviteOnlyOption(Channel *channel, char opera, std::string chain){
	std::string param;
	param.clear();
...
std::string Server::topicRestrictionOption(Channel *channel, char opera, std::string chain){
	std::string param;
	param.clear();
...
std::string Server::channelKeyOption(std::vector<std::string> tokens, Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string &arguments){
	std::string param;
	std::string pass;
...
std::string Server::operatorPrivilegeOption(std::vector<std::string> tokens, Channel *channel, size_t &pos, int fd, char opera, std::string chain, std::string &arguments){
	std::string user;
	std::string param;
...
std::string Server::userLimitOption(std::vector<std::string> tokens, Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string &arguments){
	std::string limit;
	std::string param;
	param.clear();
	limit.clear();
...
int Server::parsePartCommand(std::string cmd, std::vector<std::string> &tmp, std::string &reason, int fd){
	...
	for (size_t i = 0; i < tmp.size(); i++){
		...
		for (size_t j = 0; j < this->channels.size(); j++){
			if (this->channels[j].getName() == tmp[i]){
				...
				std::stringstream ss;
				ss << ":" << getClientFromFd(fd)->getNickname() << "!~" << getClientFromFd(fd)->getUsername() << "@"
				   << "localhost"  << " partCommand #" << tmp[i];
				if (!reason.empty())
					ss << " :" << reason << "\r\n";
				else
					ss << "\r\n";
				channels[j].sendToAll(ss.str());
				if (channels[j].getAdmin(channels[j].getClientInChannel(getClientFromFd(fd)->getNickname())->getFd()))
					channels[j].removeAdmin(channels[j].getClientInChannel(getClientFromFd(fd)->getNickname())->getFd());
				else
					channels[j].removeClient(channels[j].getClientInChannel(getClientFromFd(fd)->getNickname())->getFd());
...
void	Server::CheckForChannelsAndClients(std::vector<std::string> &tmp, int fd){
	for(size_t i = 0; i < tmp.size(); i++){
		if (tmp[i][0] == '#'){
			tmp[i].erase(tmp[i].begin());
			if(!getChannel(tmp[i])){
				sendError(401, "#" + tmp[i], getClientFromFd(fd)->getFd(), " :No such nick/channel\r\n"); 
				tmp.erase(tmp.begin() + i); 
				i--;
			} else if (!getChannel(tmp[i])->getClientInChannel(getClientFromFd(fd)->getNickname())){
				sendError(404, getClientFromFd(fd)->getNickname(), "#" + tmp[i], getClientFromFd(fd)->getFd(), " :Cannot send to channel\r\n"); 
				tmp.erase(tmp.begin() + i); 
				i--;
			} else tmp[i] = "#" + tmp[i];
		} else{
...
void	Server::privmsgCommand(std::string cmd, int fd){
	...
	if (!tmp.size()){
		sendError(411, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :No recipient given (privmsgCommand)\r\n"); 
		return;
	}
	if (message.empty()){
		sendError(412, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :No text to send\r\n"); 
		return;
	}
	if (tmp.size() > 10){
		sendError(407, getClientFromFd(fd)->getNickname(), getClientFromFd(fd)->getFd(), " :Too many recipients\r\n"); 
		return;
	}
	...
	for (size_t i = 0; i < tmp.size(); i++){
		if (tmp[i][0] == '#'){
			tmp[i].erase(tmp[i].begin());
			std::string resp = ":" + getClientFromFd(fd)->getNickname() + "!~" + getClientFromFd(fd)->getUsername() + "@localhost privmsgCommand #" + tmp[i] + " :" + message + "\r\n";
	...
	for (size_t i = 0; i < channels.size(); i++){
		if (channels[i].getClientFromFd(fd)) {
			...
			if (channels[i].getClientCount() == 0)
				...
				std::string rpl = ":" + getClientFromFd(fd)->getNickname() + "!~" + getClientFromFd(fd)->getUsername() + "@localhost quitCommand " + reason + "\r\n";
...
void Server::topicCommand(std::string &cmd, int &fd){
	...
	if (!(getChannel(nmch)->getClientFromFd(fd)) && !(getChannel(nmch)->getAdmin(fd))){
		sendError(442, "#" + nmch, fd, " :You're not on that channel\r\n");
		return;
	}
	if (scmd.size() == 2){
		if (getChannel(nmch)->getTopicName() == ""){
			sendResponse(": 331 " + getClientFromFd(fd)->getNickname() + " " + "#" + nmch + " :No topic is set\r\n", fd);
			return;
		}
		size_t pos = getChannel(nmch)->getTopicName().find(":");
		if (getChannel(nmch)->getTopicName() != "" && pos == std::string::npos)	{
			sendResponse(": 332 " + getClientFromFd(fd)->getNickname() + " " + "#" + nmch + " " + getChannel(nmch)->getTopicName() + "\r\n", fd);
			sendResponse(": 333 " + getClientFromFd(fd)->getNickname() + " " + "#" + nmch + " " + getClientFromFd(fd)->getNickname() + " " + getChannel(nmch)->getTime() + "\r\n", fd);
			return;
		} else {
			size_t pos = getChannel(nmch)->getTopicName().find(" ");
			if (pos == 0)
	...
	if (scmd.size() >= 3){
		...
		if (getChannel(nmch)->getTopicRestriction() && getChannel(nmch)->getClientFromFd(fd)){
			...
			if (pos == std::string::npos)
				rpl = ":" + getClientFromFd(fd)->getNickname() + "!" + getClientFromFd(fd)->getUsername() + "@localhost TOPIC #" + nmch + " :" + getChannel(nmch)->getTopicName() + "\r\n";
			else
				rpl = ":" + getClientFromFd(fd)->getNickname() + "!" + getClientFromFd(fd)->getUsername() + "@localhost TOPIC #" + nmch + " " + getChannel(nmch)->getTopicName() + "\r\n";
