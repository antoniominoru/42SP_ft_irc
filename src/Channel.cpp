...
void Channel::setInviteOnly(int inviteOnly) { this->inviteOnly = inviteOnly; }

void Channel::setTopic(int topic) { this->topic = topic; }

void Channel::setTime(std::string time) { this->time = time; }
...
int Channel::getInviteOnly(void) { return this->inviteOnly; }

int Channel::getTopic(void) { return this->topic; }

int Channel::getKey(void) { return this->key; }
...
bool Channel::clientInChannel(std::string &nick) {
  for (size_t i = 0; i < clients.size(); i++) {
    if (clients[i].getNickname() == nick) return true;
  }
...
std::string Channel::getTopicName(void) { return this->topicName; }

std::string Channel::getPassword(void) { return this->password; }

std::string Channel::getName(void) { return this->name; }

std::string Channel::getTime(void) { return this->time; }

std::string Channel::getCreationTime(void) { return this->createdAt; }
...
std::string Channel::getModes(void) {
  ...
  if (!mode.empty()) mode.insert(mode.begin(), '+');
  return mode;
}
...
Client *Channel::getAdmin(int fd) {
  for (std::vector<Client>::iterator it = admins.begin(); it != admins.end();
       ++it) {
    if (it->getFd() == fd) return &(*it);
  }
  return NULL;
...
Client *Channel::getClientInChannel(std::string name) {
  for (std::vector<Client>::iterator it = clients.begin(); it != clients.end();
       ++it) {
    if (it->getNickname() == name) return &(*it);
  }
  for (std::vector<Client>::iterator it = admins.begin(); it != admins.end();
       ++it) {
    if (it->getNickname() == name) return &(*it);
...
bool Channel::changeClientToAdmin(std::string &nick) {
  ...
  for (; i < clients.size(); i++) {
    if (clients[i].getNickname() == nick) break;
  }
  if (i < clients.size()) {
...
bool Channel::changeAdminToClient(std::string &nick) {
  size_t i = 0;
  for (; i < admins.size(); i++) {
    if (admins[i].getNickname() == nick) break;
  }
  if (i < admins.size()) {
