...
class Server {
 public:
  ...
  void joinExistingChannel(
      std::vector<std::pair<std::string, std::string> > &token, int i, int j,
      int fd);
  void joinNonExistingChannel(
      std::vector<std::pair<std::string, std::string> > &token, int i, int fd);
  int parseJoinCommand(std::vector<std::pair<std::string, std::string> > &token,
                       std::string cmd, int fd);
  int countJoinedChannels(std::string nickname);

  void partCommand(std::string cmd, int fd);
  int parsePartCommand(std::string cmd, std::vector<std::string> &tmp,
                       std::string &reason, int fd);

  void kickCommand(std::string cmd, int fd);
  std::string parseKickCommand(std::string cmd, std::vector<std::string> &tmp,
  ...
  std::string topicRestrictionOption(Channel *channel, char opera,
                                     std::string chain);
  std::string channelKeyOption(std::vector<std::string> splited,
                               Channel *channel, size_t &pos, char opera,
                               int fd, std::string chain,
                               std::string &arguments);
  std::string operatorPrivilegeOption(std::vector<std::string> splited,
                                      Channel *channel, size_t &pos, int fd,
                                      char opera, std::string chain,
                                      std::string &arguments);
  std::string userLimitOption(std::vector<std::string> splited,
  ...
  void getCommandArgs(std::string cmd, std::string &name, std::string &modeset,
                      std::string &params);
